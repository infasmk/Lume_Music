...
### Added ✨
- **Google Sign-In & Cloud Sync**: Added Firebase-backed Google sign-in, cloud restore, and automatic library sync for playlists, liked tracks, and listening history.
- **Profile & Account Controls**: Added a dedicated profile screen with display-name editing and sign-out support.
- **Permission & Login Gates**: Added polished permission onboarding and a cloud sign-in gate with a local-only "Continue Without Sync" path.
- **Spotify API Settings**: Added a new Spotify settings screen for custom Client ID and Client Secret so playlist imports can use user-owned Spotify Developer apps.
- **Push Notifications & Admin Broadcast Tools**: Added Firebase Cloud Messaging topic subscription, local notification display, and an internal admin sender screen for broadcast testing.
- **About Credits**: Added beta tester credits in the About screen for Navaneeth, Adhithya, Athif, Haris, Adarsh (CodeX), Adrika das, Reem, Adil, Azmeer Ajay, Anurag, Afeefa, Rishu, shaheer, and soorya.

### Changed 🔄
- **Lume Branding Refresh**: Updated launcher assets, package identifiers, and related branding for the Lume release.
- **Runtime Version Labels**: Loading, auth, and About screens now read the installed app version dynamically instead of using hardcoded text.
- **Cloud Feature Bootstrapping**: Firebase-backed features now initialize in an optional-safe way so the app can still run when Firebase is not configured.
- **Spotify Import Flow**: Spotify token handling now prefers saved app credentials from settings instead of only bundled defaults.

### Fixed 🛠️
- **Spotify Import Error Reporting**: Import failures now show the real Spotify/API error instead of a generic failure label.
- **Cross-Account Sync Isolation**: Fixed cloud restore and sync edge cases so switching accounts does not leak local library data between users.
- **Notification & Media Permission Flow**: Improved permission handling for Android and iOS startup so required access requests are clearer and more reliable.
- **Version Drift in UI**: Fixed version mismatches across app surfaces by removing stale hardcoded version strings.

### Removed 🧹
- **First-Launch Welcome Popup**: Removed the newly added first-launch popup from startup.
